...
const std::vector<std::string> JETBRAINS_PRODUCT =
{
    "IntelliJIdea",
    "PyCharm",
    "WebStorm",
...
void reset(const std::filesystem::path& product)
{
    ...
    if (const std::filesystem::path key(product / "clion.key"); std::filesystem::exists(key))
    {
        ...
        {
            std::cout << "[" << GREEN "+" << RESET << "] Removed Trial License Key " << GREEN << key.filename() << RESET << std::endl;
        }
        else
        {
...
{
#ifdef _WIN32
    for (const std::filesystem::path path = std::filesystem::path(home()) / "JetBrains"; const auto& entry : std::filesystem::recursive_directory_iterator(path))
    {
        if (entry.is_regular_file())
        {
            ...
            if (const auto it = std::ranges::find(JETBRAINS_IDS_FILE, stem); it != JETBRAINS_IDS_FILE.end())
            {
                ...
                {
                    std::cout << "[" << GREEN << "+" << RESET << "] Purged File '" << GREEN << entry.path().filename() << RESET << "'." << std::endl;
                }
            }
        }
    }
    ...
    {
        if (const LSTATUS status = RegDeleteTreeA(HKEY_CURRENT_USER, target.c_str()); status == ERROR_SUCCESS)
        {
            std::cout << "[" << GREEN << "+" << RESET << "] Purged Registry  Key '" << GREEN << target << RESET << "'." << std::endl;
        }
        ...
        {
            std::cout << "[" << RED << "X" << RESET << "] Could not delete '" << GREEN << target << RESET << "' Code: " << YELLOW << status << RESET << "." << std::endl;
        }
    }
#else
...
int main(const int argc, char** argv)
{
    ...
    add("-t", [&base](const auto& args) {
        ...
        for (const auto& entry : JETBRAINS_REGISTRY_PATH)
        {
            ...
            if (const LSTATUS status = RegOpenKeyExA(HKEY_CURRENT_USER, entry.c_str(), 0, KEY_READ, &key); status == ERROR_SUCCESS)
            {
